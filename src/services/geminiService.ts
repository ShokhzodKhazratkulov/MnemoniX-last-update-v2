...
export class GeminiService {
  ...
  constructor() {
    ...
    // AI Studio injects GEMINI_API_KEY automatically, others must have VITE_ prefix
    ...
    if (keysString && keysString !== 'undefined') {
      ...
      console.log(`GeminiService: Loaded ${loadedKeys.length} keys from VITE_GEMINI_API_KEYS`);
    }
    ...
    if (individualKeys.length > 0) {
      console.log(`GeminiService: Found ${individualKeys.length} individual keys`);
    }
    ...
    if (this.apiKeys.length === 0) {
      ...
      console.log("Available env keys:", Object.keys(import.meta.env).filter(k => k.includes('GEMINI')));
    } else {
  ...
  private rotateKey() {
    if (this.apiKeys.length > 1) {
      ...
      console.warn(`Quota reached. Rotating to API Key #${this.currentKeyIndex + 1} (Project ${this.currentKeyIndex + 1})`);
    }
  }
  ...
  private async withRetry<T>(fn: () => Promise<T>, maxRetries = 5): Promise<T> {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (isQuotaError) {
          this.rotateKey(); // Switch to the next project's key immediately
        }
